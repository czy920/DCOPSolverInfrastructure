...
public class MaxsumADSSVPAgent extends SyncAgent{

    private static final int MAX_CYCLE = 4800;
    private static final int LONGEST_PATH_LENGTH = 240;
    private static final int TIMING = 2;
    private static final int VALUE_PROPAGATION_LENGTH = 2;
    private static final int BELIEF_PROPAGATION_LENGTH = 1;
    ...
    public MaxsumADSSVPAgent(int id, int[] domain, int[] neighbours, Map<Integer, int[][]> constraintCosts, Map<Integer, int[]> neighbourDomains, SyncMailer mailer) {
    ...
    public void allMessageDisposed() {
        ...
        switch (phase){
            case PHASE_Q:
                    ...
                    if (round >= TIMING && (round - TIMING) % (VALUE_PROPAGATION_LENGTH + BELIEF_PROPAGATION_LENGTH) < VALUE_PROPAGATION_LENGTH) {
